"""
analyzer/types_features.py
==========================
すべての音響特徴量データクラスおよびシリアライズ/変換ヘルパー関数を定義しますの。
純粋な型定義レイヤーとして副作用を一切持ちませんわ！
"""
...
# ─────────────────────────────────────────────
# 新規特徴量データクラス群 (Step 3 連携用)
# ─────────────────────────────────────────────
@dataclass
class PsychoacousticsFeatures:
    """DIN 45692 聴覚心理音響特徴量。"""

    sharpness_acum: float = 0.0
    roughness_asper: float = 0.0
    tonality_val: float = 0.0
    specific_loudness_seq: list[float] = field(default_factory=list)


@dataclass
class StructureSsmFeatures:
    """SSM / 楽曲構造展開特徴量。"""

    chorus_start_sec: float = 0.0
    chorus_end_sec: float = 0.0
    drop_position_sec: float = 0.0
    structural_complexity: float = 0.0
    novelty_seq: list[float] = field(default_factory=list)


@dataclass
class VoiceCppFeatures:
    """ボーカルケプストラムピーク突出度 (CPP / CPPS)。"""

    cpp_mean: float = 0.0
    cpp_std: float = 0.0
    cpp_seq: list[float] = field(default_factory=list)
    breathiness_score: float = 0.0


@dataclass
class AudioCutoffLufsFeatures:
    """高域カットオフ・True Peak・EBU R128 特徴量。"""

    cutoff_frequency_hz: float = 0.0
    cutoff_steepness_db_oct: float = 0.0
    is_fake_hires: bool = False
    true_peak_dbtp: float = 0.0
    integrated_lufs: float = 0.0
    loudness_range_lra: float = 0.0


# ─────────────────────────────────────────────
# RawFeatures (統合データクラス)
# ─────────────────────────────────────────────
@dataclass
class RawFeatures:
    """特徴量統合データクラスですわ！"""
    ...
    # 新規拡張
    psychoacoustics_feat: PsychoacousticsFeatures | None = None
    structure_ssm_feat: StructureSsmFeatures | None = None
    voice_cpp_feat: VoiceCppFeatures | None = None
    audio_cutoff_lufs_feat: AudioCutoffLufsFeatures | None = None
    ...
    def __post_init__(self):
        # hnr_db が設定されていて hnr がデフォルト値の場合は同期
        if self.hnr_db != 0.0 and self.hnr == 0.0:
            self.hnr = self.hnr_db
        elif self.hnr != 0.0 and self.hnr_db == 0.0:
            self.hnr_db = self.hnr

    def to_dict(self) -> dict[str, Any]:
        """後方互換用: Postgres 辞書フォーマット (scalars / sequences) を返却しますわ。"""
        return {
            "scalars": _stem_filter_scalars(self, "mix"),
            "sequences": _stem_filter_sequences(self, "mix"),
        }


class LibrosaFeatures(RawFeatures):
    """旧バージョンとの 100% 互換性を持つデータクラスですわ。"""

    def __init__(self, **kwargs):
        # レガシー引数名のマッピング
        if "spectral_centroid_mean" in kwargs:
            kwargs["centroid_mean"] = kwargs.pop("spectral_centroid_mean")
        if "spectral_centroid_sd" in kwargs:
            kwargs["centroid_std"] = kwargs.pop("spectral_centroid_sd")
        if "rolloff" in kwargs:
            kwargs["rolloff_mean"] = kwargs.pop("rolloff")
        if "zcr" in kwargs:
            kwargs["zcr_mean"] = kwargs.pop("zcr")
        if "hnr_db" in kwargs:
            kwargs["hnr"] = kwargs["hnr_db"]
...
def _stem_filter_scalars(raw: RawFeatures, track_id: str) -> dict[str, Any]:
    ...
    # 新規拡張スカラー
    if raw.psychoacoustics_feat is not None:
        scalars["psychoacoustics"] = {
            "sharpness_acum": float(raw.psychoacoustics_feat.sharpness_acum),
            "roughness_asper": float(raw.psychoacoustics_feat.roughness_asper),
            "tonality": float(raw.psychoacoustics_feat.tonality_val),
        }
    if raw.structure_ssm_feat is not None:
        scalars["structure_ssm"] = {
            "chorus_start_sec": float(raw.structure_ssm_feat.chorus_start_sec),
            "chorus_end_sec": float(raw.structure_ssm_feat.chorus_end_sec),
            "drop_position_sec": float(raw.structure_ssm_feat.drop_position_sec),
            "structural_complexity": float(
                raw.structure_ssm_feat.structural_complexity
            ),
        }
    if raw.voice_cpp_feat is not None:
        scalars["voice_cpp"] = {
            "cpp_mean": float(raw.voice_cpp_feat.cpp_mean),
            "cpp_std": float(raw.voice_cpp_feat.cpp_std),
            "breathiness_score": float(raw.voice_cpp_feat.breathiness_score),
        }
    if raw.audio_cutoff_lufs_feat is not None:
        scalars["audio_cutoff_lufs"] = {
            "cutoff_frequency_hz": float(
                raw.audio_cutoff_lufs_feat.cutoff_frequency_hz
            ),
            "cutoff_steepness_db_oct": float(
                raw.audio_cutoff_lufs_feat.cutoff_steepness_db_oct
            ),
            "is_fake_hires": bool(raw.audio_cutoff_lufs_feat.is_fake_hires),
            "true_peak_dbtp": float(raw.audio_cutoff_lufs_feat.true_peak_dbtp),
            "integrated_lufs": float(raw.audio_cutoff_lufs_feat.integrated_lufs),
            "loudness_range_lra": float(raw.audio_cutoff_lufs_feat.loudness_range_lra),
        }
    return scalars
...
def _stem_filter_sequences(raw: RawFeatures, track_id: str) -> dict[str, Any]:
    ...
    # 新規拡張シーケンス
    if raw.psychoacoustics_feat and raw.psychoacoustics_feat.specific_loudness_seq:
        sequences["specific_loudness"] = (
            raw.psychoacoustics_feat.specific_loudness_seq
        )
    if raw.structure_ssm_feat and raw.structure_ssm_feat.novelty_seq:
        sequences["structure_novelty"] = raw.structure_ssm_feat.novelty_seq
    if raw.voice_cpp_feat and raw.voice_cpp_feat.cpp_seq:
        sequences["voice_cpp_seq"] = raw.voice_cpp_feat.cpp_seq
