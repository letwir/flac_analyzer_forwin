...
本プロジェクト (`flac_analyzer_forwin`) では、パイプライン本体の実行・運用・データマイグレーション・検証・タグ修復・進捗可視化を支援する各種独立治具スクリプトを `zig/` フォルダに集約・整備しておりますわ。
...
| :--- | :--- | :--- |
| [`check_tag_consistency.py`](file:///a:/Users/letwir/repo/flac_analyzer_forwin/zig/check_tag_consistency.py) | DB ⇔ FLAC タグの双方向整合性チェッカー＆一括修復（DB→FLAC未反映タグ検出、FLAC→DBメタデータ差分、双方向diff、JSONレポート） | `python zig/check_tag_consistency.py --dir M:\Music --mode diff`<br>`python zig/check_tag_consistency.py --repair` |
| [`dashboard.py`](file:///a:/Users/letwir/repo/flac_analyzer_forwin/zig/dashboard.py) | Prometheus `/metrics` (:2112) をリアルタイム取得し、1ファイル/1曲所要時間・スループット・ETA・RAM/ディスク残量を描画する TUI ダッシュボード | `python zig/dashboard.py`<br>`python zig/dashboard.py --once` |
| [`repair_flac_tags.py`](file:///a:/Users/letwir/repo/flac_analyzer_forwin/zig/repair_flac_tags.py) | PostgreSQL DB 内の解析結果から FLAC 本体タグへの不足分一括焼き戻し・修復（重複排除＆Windowsタイムスタンプ保護） | `python zig/repair_flac_tags.py --dir M:\Music --limit 100` |
...
### 1. `check_tag_consistency.py`
- **目的**: Issue #15 に基づく DB (`raw.library_flac`) と FLAC ファイル（VorbisComment）の双方向整合性検査および一括修復。
- **モード**:
  - `db-to-flac`: DB 内の解析結果（Librosa, Essentia 453, Tensor）が FLAC タグに存在するか検証し、`--repair` で不足タグを一括焼き込み。
  - `flac-to-db`: FLAC にメタデータ・解析タグがあるが DB に未登録/差分があるものを検出し、レポート・修復。
  - `diff` / `both`: 双方向クロスチェックを行い、不一致タイプ別の詳細差分サマリーおよび `--output-json` で JSON レポートを出力。
- **オプション**: `--dir`, `--limit`, `--dry-run`, `--repair`, `--output-json`。

### 2. `dashboard.py`
- **目的**: Issue #16 に基づくリアルタイム CLI 進捗ダッシュボード（TUI）。
- **特徴**:
  - Orchestrator の Prometheus エンドポイント (`http://localhost:2112/metrics`) からデータを取得。
  - **1ファイルあたり所要時間**（平均・直近）および **1曲（トラック）あたり所要時間**（平均・直近）を高精度に表示。
  - 処理スループット（`files/min`, `tracks/min`）、ETA（残り推定時間）、待機キュー長、稼働ワーカー数、Demucs スロット使用数、RAM / ディスク空き容量をリアルタイム更新。
  - Rich による美麗なコンソール TUI および ANSI フォールバックモード完備。

### 3. `repair_flac_tags.py`
