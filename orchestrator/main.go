...
func worker(id int, taskQueue <-chan TaskPayload, wg *sync.WaitGroup, noDB bool) {
	...
	for task := range taskQueue {
		...
		// 4.5 Run Precache Functor
		log.Printf("%s[W-%d] [IO Monad] Running Precache Functor...%s\n", ColorCyan, id, ColorReset)
		
		cmdPrecache := exec.Command(pythonPath, "functor_precache.py", "--shm-metadata", demucsMetaJson, "--track-hash", trackHash)
		cmdPrecache.Dir = parentDir
		cmdPrecache.Env = append(os.Environ(), envVars...)
		
		var precacheOutBuf bytes.Buffer
		cmdPrecache.Stdout = &precacheOutBuf
		stderrPrecache, _ := cmdPrecache.StderrPipe()
		
		if err := cmdPrecache.Start(); err != nil {
			log.Printf("%s[W-%d] [IO Monad] Precache start failed: %v%s\n", ColorRed, id, err, ColorReset)
			for _, shm := range shmMap {
				shm.Close()
			}
			continue
		...
		streamColoredLog(stderrPrecache, id, "Precache", ColorCyan)
		
		if errPrecache := cmdPrecache.Wait(); errPrecache != nil {
			log.Printf("%s[W-%d] [IO Monad] Precache processing failed: %v%s\n", ColorRed, id, errPrecache, ColorReset)
			for _, shm := range shmMap {
				shm.Close()
			}
			continue
		...
		// Update meta JSON with the new one generated by Precache Functor (contains cached .npy paths)
		precacheMetaJson := precacheOutBuf.String()
		log.Printf("%s[W-%d] [IO Monad] Precache completed successfully.%s\n", ColorGreen, id, ColorReset)
		
		// 5. Run Librosa Worker
		log.Printf("%s[W-%d] [IO Monad] Running Librosa worker...%s\n", ColorPurple, id, ColorReset)
		
		cmdLibrosa := exec.Command(pythonPath, "worker_librosa.py", "--shm-metadata", precacheMetaJson, "--track-hash", trackHash)
		...
		cmdTensor := exec.Command(pythonPath, "worker_tensor.py", "--shm-metadata", precacheMetaJson, "--track-hash", trackHash)
		...
		cmdEssentia := exec.Command(pythonPath, "worker_essentia.py", "--shm-metadata", precacheMetaJson, "--track-hash", trackHash)
