...
使い方:
    # 単体値の確認
    python zig/migrate_hnr.py --calc-db 0.85
    python zig/migrate_hnr.py --calc-nap 7.54

    # DB マイグレーションのドライラン (確認のみ)
    python zig/migrate_hnr.py --dry-run

    # 実際の DB 更新実行 (バッチコミット)
    python zig/migrate_hnr.py --batch-size 500

    # DB 更新 ＋ FLAC タグ同期書き戻し
    python zig/migrate_hnr.py --fix-tags --batch-size 500
"""
    ...
    if sys.platform == "win32" and ctime is not None:
        try:
            ...
            if handle != -1 and handle != 0:
                ...
                ft_creation = FILETIME(ft_low, ft_high)
                ctypes.windll.kernel32.SetFileTime(
                    handle, ctypes.byref(ft_creation), None, None
                )
                ctypes.windll.kernel32.CloseHandle(handle)
        except Exception as e:
            logging.warning(f"Win32 SetFileTime 復元中に失敗いたしましたわ: {e}")
