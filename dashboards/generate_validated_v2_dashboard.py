...
# Load sample as exact template
with open('parsed_user_sample.json', 'r', encoding='utf-8') as f:
    sample = json.load(f)

# Build the complete FLAC Analyzer Dashboard in exact Grafana 13 / Scenes v2 schema
dashboard = {
  "apiVersion": "dashboard.grafana.app/v2",
  "kind": "Dashboard",
  "metadata": {
    "name": sample['metadata'].get('name', 'flac-analyzer')
  },
  "spec": {
    "annotations": [
      {
    ...
    "cursorSync": "Off",
    "description": "FLAC Analyzer 音響解析パイプライン可観測性ダッシュボード",
    "editable": True,
    "elements": {
      "panel-1": {
        "kind": "Panel",
...
print(f"Generated validated Scenes v2 dashboard at {out_path} ({len(json.dumps(dashboard))} bytes)")
